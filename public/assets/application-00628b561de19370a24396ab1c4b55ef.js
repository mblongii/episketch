...
(function() {
  $(function() {
    var el, i, j, len, ref, results, sketch, sort_sketches;
      ...
      sort_sketches = Sortable.create(el = $("#sketches")[0], {
        ...
        store: {
          ...
          set: function(sortable) {
            ...
            return localStorage.setItem(sortable.options.group, order.join(','));
          }
        }
      });
      $(window).on("scrollstart", function() {
        return sort_sketches.option('disabled', true);
      }).on("scrollstop", function() {
        return sort_sketches.option('disabled', false);
      });
      i = 0;
      ref = $('.sketch-img');
      results = [];
...
(function() {
  $(function() {
    var colorThief, color_src, i, image_loading_delay, j, len, ref, sampled_image, sketch;
    if ($('.swatch_test.index').length > 0) {
      console.log('swatch test');
      $('#swatch_test').hide();
      image_loading_delay = setTimeout(function() {
        return $('#loading_modal').modal('show');
      });
      300;
      ...
      return $(color_src).load(function() {
        var bg_color, brightest, color_swatch, darkest, desc_color, k, l, len1, len2, len3, m, next_brightest, num_colors, results, rgb, sum, sums, the_color, the_palette, title_color;
        clearTimeout(image_loading_delay);
        $('#swatch_test').show();
        $('#loading_modal').modal('hide');
        num_colors = 8;
        the_color = colorThief.getColor(color_src);
        the_palette = colorThief.getPalette(color_src, num_colors);
        for (k = 0, len1 = the_palette.length; k < len1; k++) {
          rgb = the_palette[k];
        ...
        sums = [];
        for (l = 0, len2 = the_palette.length; l < len2; l++) {
          rgb = the_palette[l];
          sum = rgb.reduce(function(x, y, z) {
            return x + y + z;
          });
          sums.push(sum);
        }
        ...
        results = [];
        for (m = 0, len3 = the_palette.length; m < len3; m++) {
          rgb = the_palette[m];
          color_swatch = $("<div class='swatch_" + (i++) + "'>");
          color_swatch.attr("style", "background-color: rgb(" + rgb + "); width: 20px; height: 20px; border: 1px solid gray; display: inline-block;");
          color_swatch.attr("title", "" + rgb);
          results.push(color_swatch.prependTo('#swatch_test'));
        }
        return results;
      });
    }
